...
TEST(gauss_method, can_calculate_matrix_correct_using_sequential_method) {
    ...
    if (rank == 0) {
        ...
        EXPECT_NEAR(res[0], -0.2, 0.00001); // исправить погрешность
...
TEST(gauss_method, can_calculate_matrix_with_size_in_5_elements) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    std::vector<std::vector<double>> vec{ {1, 2, 2, 3, 5}, {8, 7, 2, 2, 6}, {5, 3, 3, 1, 2},  {2, 5, 8, 4, 6}, {3, 2, 1, 7, 5} };
    Matrix mat(vec);
    std::vector<double> res(mat.getMatrixSize());
    res = mat.getParallelSolution({ 5, 3, 7, 4, 6 });
    if (rank == 0) {
        EXPECT_NEAR(res[0], 2.533, 0.001); // исправить погрешность
