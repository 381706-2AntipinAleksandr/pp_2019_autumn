...
std::vector<int> merge_batcher(std::vector<int> global_vec, int size_vec) {
  ...
  MPI_Scatterv(global_vec.data(), countsend, dis, MPI_INT, &local_vec.front(), countsend[rank], MPI_INT, 0, MPI_COMM_WORLD);
  ...
  for (int i = 0; i < numder; i++) {
    if ((rank % merged_proc == 0) && (rank + displs_proc < size)) {
      ...
      MPI_Recv(&odd.front(), length_recv / 2 + local_vec.size() / 2, MPI_INT, rank + displs_proc, 0, MPI_COMM_WORLD, &status);
